...
def parse_args():
    ...
    parser.add_argument('--data_dir', type=str, default='data/2x5')
    ...
    parser.add_argument('--lamb_bfp', type=float, default=1.9717)
    parser.add_argument('--lamb_bfp_update_freq', type=int, default=300)
    parser.add_argument('--rho_bfp', type=float, default=0.55)
    parser.add_argument('--rho_bfp_update_freq', type=int, default=800)
    parser.add_argument('--delta_rho_bfp', type=float, default=0.05)


    parser.add_argument('--n_layer', type=int, default=5)
    parser.add_argument('--n_head', type=int, default=8)
    parser.add_argument('--d_hidden', type=int, default=64)
    parser.add_argument('--init_softmax_temperature', type=int, default=5000)
    ...
    parser.add_argument('--decay_round_one', type=int, default=4000)  #
...
if __name__ == "__main__":
    ...
    rho_bfp = args.rho_bfp
    lamb_bfp = args.lamb_bfp * torch.ones(1).to(DEVICE)

    train_bfp_record=[]
    test_bfp_record=[]

    train_vsp_utlity_record=[]
    test_vsp_utlity_record=[]

    train_user_utlity_record = []
    test_user_utlity_record = []

    model = RAMANet(args).to(DEVICE)
    # if args.bool_test:
    state_dict = torch.load('model/5x2')
    ...
    optimizer = torch.optim.Adam(model.mechanism.parameters(), lr=args.two_lr)#warm_up_init
    ...
    for i in tqdm(range(args.train_steps)):  # parser.add_argument('--train_steps', type=int, default=2000)
        # if i==2000:
        #     torch.save(model.mechanism.state_dict(), "2000")
        if i == args.train_steps - 1 or ((
            ...
            with torch.no_grad():
                ...
                test_bfp=torch.zeros(1).to(DEVICE)
                for num in range(int(test_values.shape[0] / bs)):
                    ...
                    test_cost += cost.sum()
                    test_bfp += torch.relu(payment.sum(0) - args.budget).sum()

                test_utility /= test_values.shape[0]
                test_payment /= test_values.shape[0]
                test_cost /= test_values.shape[0]
                test_bfp /= test_values.shape[0]
                logger.info(
                    f"step {i}: test_sp_utility: {test_utility}," f"test_user_utility: {test_payment-test_cost}," f"test_payment: {test_payment}," f"test_cost: {test_cost}," f"test_bfp: {test_bfp}")
                test_bfp_record.append(test_bfp.item())
                test_vsp_utlity_record.append(test_utility.item())
                test_user_utlity_record.append((test_payment-test_cost).item())
        ...
        for num in range(num_per_train):  # train num_per_train = int(args.train_sample_num / bs)
            ...
            B = torch.relu(torch.sum(payment, dim=0) - torch.tensor(args.budget))  # (bs,)
            B_Mean = torch.mean(B).to(DEVICE)
            lagrangianLoss_B = torch.sum(B_Mean * lamb_bfp).to(DEVICE)
            lagLoss_B = (rho_bfp / 2) * torch.sum(torch.pow(B_Mean, 2)).to(DEVICE)
            ...
            loss = (- (utility.sum(0).mean() + (payment.sum(0).mean() - cost.sum(0).mean())) \
                   ...
                   + 0.5 * torch.abs((1 - args.gama) * (utility.sum(0) + payment.sum(0) - cost.sum(0)) - (payment.sum(0) - cost.sum(0))).mean())+ lagrangianLoss_B + lagLoss_B
            ...
            train_bfp += torch.relu(payment.sum(0) - args.budget).mean().data
            # train_op+=op.data
            loss.backward()
            optimizer.step()
        ...
        if i % 1 == 0:
            logger.info(f"step {i}: loss: {reportloss / num_per_train},"
                        ...
                        f"train_bfp: {train_bfp / num_per_train},"
                        f"lamb_bfp: {lamb_bfp},"
                        f"rho_bfp: {rho_bfp}")
            train_bfp_record.append((train_bfp / num_per_train).item())
            train_vsp_utlity_record.append((train_utility / num_per_train).item())
            train_user_utlity_record.append(((train_payment -train_cost)/ num_per_train).item())
        ...
        if  (i+1) % args.lamb_bfp_update_freq == 0:
            lamb_bfp += rho_bfp * (train_bfp/ num_per_train)

        if  (i+1) % args.rho_bfp_update_freq == 0:
            rho_bfp += args.delta_rho_bfp


    df_train_bfp = pd.DataFrame(np.array(train_bfp_record))
    df_train_vsp_utlity_record=pd.DataFrame(np.array( train_vsp_utlity_record))
    # train_vsp_utlity_record.append((train_utility / num_per_train).item())
    df_train_user_utlity_record=pd.DataFrame(np.array(train_user_utlity_record))
    # train_user_utlity_record.append(((train_payment - train_cost) / num_per_train).item())


    df_test_bfp = pd.DataFrame(np.array(test_bfp_record))
    df_test_vsp_utlity_record=pd.DataFrame(np.array(test_vsp_utlity_record))
    df_test_user_utlity_record=pd.DataFrame(np.array(test_user_utlity_record))
    # test_vsp_utlity_record.append(test_utility.item())
    # test_user_utlity_record.append((test_payment - test_cost).item())


    df_train_bfp.to_csv("train_bfp")
    df_train_vsp_utlity_record.to_csv("train_vsp_utility")
    df_train_user_utlity_record.to_csv("train_user_utility")

    df_test_bfp.to_csv("test_bfp")
    df_train_vsp_utlity_record.to_csv("test_vsp_utility")
    df_train_user_utlity_record.to_csv("test_user_utility")
    ...
    with torch.no_grad():
        ...
        final_test_bfp = torch.zeros(1).to(DEVICE)
        for num in range(int(final_test_values.shape[0] / bs)):
            ...
            final_test_bfp += torch.relu(payment.sum(0) - args.budget).sum()
        ...
        final_test_bfp /= final_test_values.shape[0]
        logger.info(
            ...
            f"final_test_bfp: {final_test_bfp}")
